...
>(({
  ...
  const handleRunTest = async () => {
    ...
    try {
      ...
      for (let i = 0; i < testCases.length; i++) {
        ...
        const result = isSql
          ? await runSqlCode(userCode, testCase.codeRun)
          : await runCode(userCode, testCase.codeRun);
        results[i] = result;
      }
  ...
  // Function to run user SQL against an isolated in-browser Postgres (psql-style) instance
  const runSqlCode = async (userCode: string, testExpression: string) => {
    let db: InstanceType<typeof import('@electric-sql/pglite').PGlite> | null = null;
    try {
      ...
      const result = await db.query(testExpression);
      return result.rows;
    } catch (error) {
      return `Error: ${
        error instanceof Error ? error.message : String(error)
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex gap-1">
              {testCases.map((testCase, index) => {
                ...
                const passed = hasResult && !isTypeError && areValuesEqual(testResults[index], testCase.expected);
          ...
          {testCases[selectedTestCase] && (
            <>
              ...
              <div>
                ...
                <div className="exercise-terminal-expected-output rounded p-3 font-mono text-sm wrap-break-word whitespace-pre-wrap">
                  {formatDisplayValue(testCases[selectedTestCase].expected)}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="exercise-terminal-output rounded p-3 font-mono text-sm  wrap-break-word whitespace-pre-wrap">
                  {testResults[selectedTestCase] !== undefined ? (() => {
                    ...
                    const isMatch = !isTypeError && areValuesEqual(output, testCases[selectedTestCase].expected);
                    return (
