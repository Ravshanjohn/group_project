...
export const login = async ( req: Request, res: Response ) => {
  try {
    ...
    // Basic validation
    if(!email || !password) return res.status(400).json({ data: null, error: { code: 'credentials_required', message: 'Email and password are required' } });
    ...
    const { data: user, error: userError } = await database
      .from('users')
      .select('id, first_name, last_name, email, password, avatar, balance, is_verified, active_status, created_at, deleted_at')
    ...
    if (userError || !user){
      return res.status(400).json({ data: null, error: { code: 'invalid_credentials', message: 'Invalid email or password' } });
    };

    if(!user.password) {
      return res.status(500).json({ data: null, error: { code: 'incomplete_account', message: 'Account data is incomplete. Please contact support.' } });
    };
    
    const isPasswordValid = await bcrypt.compare(password, user.password);

    if(!isPasswordValid) return res.status(400).json({ data: null, error: { code: 'invalid_credentials', message: 'Invalid email or password' } });


    if(!user.is_verified) {
      return res.status(403).json({ data: null, error: { code: 'email_not_verified', message: 'Email is not verified. Please check your inbox.' } });
    };

    if(user.deleted_at) {
      return res.status(403).json({ data: null, error: { code: 'account_deactivated', message: `Account is ${user.active_status}. Please contact support.` } });
    };
...
export const signup = async ( req: Request, res: Response ) => {
  try {
    const { firstName, lastName, first_name, last_name, email, password } : { firstName?: s, lastName?: s | null, first_name?: s, last_name?: s | null, email: s, password: n } = req.body;

    const resolvedFirstName = firstName ?? first_name;
    const resolvedLastName = lastName ?? last_name;

    // Basic validation
    if(!resolvedFirstName || !email || !password) return res.status(400).json({ data: null, error: { code: 'fields_required', message: 'All fields are required' } });
    if( password.toString().length < 6 ) return res.status(400).json({ data: null, error: { code: 'password_too_short', message: 'Password must be at least 6 characters long' } });

    // Trim and normalize inputs
    const trimmedEmail = email.trim().toLowerCase();
    const trimmedFirstName = resolvedFirstName.trim();
    const trimmedLastName = resolvedLastName ? resolvedLastName.trim() : null;
    ...
    if (userExists) {
      ...
      return res.status(409).json({ data: null, error: { code: 'user_exists', message: 'Email verification sent if account does not exists. Please check your inbox.' } });
    };
...
export const verifyAccount = async ( req: Request, res: Response ) => {
  try {
    ...
    if (!token) return res.status(400).json({ data: null, error: { code: 'token_required', message: "Token is required" } });
    const hashedToken = crypto.createHash('sha256').update(token).digest('hex');
    ...
    if (!Array.isArray(tokenRecord) || tokenRecord.length === 0) {
      return res.status(400).json({ data: null, error: { code: 'invalid_token', message: "Invalid or expired token" } });
    };
...
export const verifyEmail = async ( req: Request, res: Response) => {
  try {
    ...
    //Validate email
    if (!email) return res.status(400).json({ data: null, error: { code: 'email_required', message: "Email is required" } });
    const trimmedEmail = email.trim().toLowerCase();
    ...
    if (!user) return res.status(404).json({ data: null, error: { code: 'user_not_found', message: "User not found" } });
    if (user.is_verified) return res.status(400).json({ data: null, error: { code: 'email_already_verified', message: "Email is already verified" } });
    if (user.deleted_at) return res.status(400).json({ data: null, error: { code: 'account_deleted', message: "Account has been permanently deleted.Please contact support if you believe this is an error." } });
    ...
    if (canSend === true) {
      ...
      if (!isSent) {
        ...
        return res.status(400).json({ data: null, error: { code: 'verification_email_failed', message: "Message is not sent (Error in verifyEmail controller)" } });
      };

      return res.status(200).json({ data: null, error: null });
    } else {
      //Email cannot be sent due to limits
      return res.status(400).json({ data: null, error: { code: 'email_limit_reached', message: 'Email limit reached or cooldown active' } });
    };
  } catch (error: unknown) {
...
export const forgotPassword = async ( req: Request, res: Response ) => {
  ...
  try {
    ...
    if (!user) return res.status(404).json({ data: null, error: { code: 'user_not_found', message: "User not found" } });
    if (!user.is_verified) return res.status(403).json({ data: null, error: { code: 'email_not_verified', message: "Email is not verified" } });
    if (user.deleted_at) return res.status(403).json({ data: null, error: { code: 'account_deactivated', message: "Account is deactivated. Please contact support." } });
    ...
    if (canSend === true) {
      ...
      if (!isSent) {
        ...
        return res.status(400).json({ data: null, error: { code: 'reset_email_failed', message: "Message is not sent" } });
      };
...
export const resetPassword = async ( req: Request, res: Response ) => {
  try {
    const { token } = req.params as { token: s } ;
    const { newPassword, new_password } : { newPassword?: s, new_password?: s } = req.body;
    const resolvedPassword = newPassword ?? new_password;

    // Basic validation
    if(!resolvedPassword) return res.status(400).json({ data: null, error: { code: 'password_required', message: 'New password is required' } });
    if( resolvedPassword.toString().length < 6 ) return res.status(400).json({ data: null, error: { code: 'password_too_short', message: 'Password must be at least 6 characters long' } });
    if(!token) return res.status(400).json({ data: null, error: { code: 'invalid_token', message: 'Token is invalid or expired' } });
    ...
    const hashedPassword: string = await bcrypt.hash(resolvedPassword, 10);
    const now = new Date();
...
export const checkAuthStatus = async (req: Request, res: Response) => {
	try {
    return res.status(200).json({ data: (req as any).user, error: null });
	} catch (error: unknown) {
